...
const mockCriteria = () => ([
  {
    ...
    type: "select",
    values: "died as a result of a service-connected disability; died while receiving or traveling to receive VA care; died while eligible, pending to receive, or receiving VA compensation or pension",
  },
  {
...
describe('criteria', () => {
  describe('mutations', () => {
    it('should preloaded responses', () => {
      ...
      mutations.populateCriterion(storeState, { hash: criteria[0].criteriaKeyHash, criterion: criteria[0] });
      mutations.preloadedResponse(storeState, { response: true, criteriaKeyHash: criteria[0].criteriaKeyHash});
      mutations.preloadedResponse(storeState, { response: 'spouse', criteriaKeyHash: criteria[1].criteriaKeyHash});
      mutations.populateCriterion(storeState, { hash: criteria[1].criteriaKeyHash, criterion: criteria[1] });
      expect(storeState.eligibilityCriteria[criteria[0].criteriaKey].response).toBe(true);
      expect(storeState.preloadedResponses[criteria[1].criteriaKeyHash]).toBe('spouse');
    ...
    it('should populate criterion', () => {
      ...
      mutations.populateCriterion(storeState, { hash: criterion.criteriaKeyHash, criterion });
